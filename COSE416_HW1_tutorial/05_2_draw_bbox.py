...
####################2가지 선택지 (OPTICS, DBSCAN) 클러스터링 ##############
##1. OPTICS 클러스터링 적용
# from sklearn.cluster import OPTICS
# optics = OPTICS(min_samples=10, xi=0.05, min_cluster_size=0.1)
# labels = optics.fit_predict(final_point)
# 결과 시각화
# unique_labels = set(labels)
# print(f"Detected {len(unique_labels) - 1} clusters (excluding noise).")
...
min_points_in_cluster = 0   # 클러스터 내 최소 포인트 수
max_points_in_cluster = 50  # 클러스터 내 최대 포인트 수
...
for feature in features:
    if 0.0 <= feature["width"] <= 1.2 and \
       + 0.8 <= feature["height"] <= 2.2 and \
       + 0.0 <= feature["depth"] <= 1.0 and \
       + 0.05 <= feature["density"] <= 50.0 and \
       + -1.5 <= feature["z_min"] and \
       + 3.0 >= feature["z_max"] and \
       + 50.0 >= feature["max_distance"] and \
       + 1 <= feature["points_in_cluster"] <= 100:
