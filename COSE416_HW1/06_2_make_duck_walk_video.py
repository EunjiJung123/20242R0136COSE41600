# video로 나타낸 data 중 '02_straight_duck_walk'은 이 코드로 수행
# 시각화에 필요한 라이브러리 불러오기
...
# pcd_files = pcd_files[-20:] # test code
...
# 카메라 위치 및 뷰포인트 설정 함수
def set_camera_view(vis, zoom=0.15, front=[0, -1, 0.1], lookat=[0, 20, 0], up=[0, 0, 1]):
...
movement_threshold = 0.4  # 클러스터가 움직였다고 판단하는 최소 이동 거리
